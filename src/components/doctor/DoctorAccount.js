...
function DoctorAccount() {
    ...
    const handleSignUp = async () => {
        try {
            if (validation()) {
                let doctorName = nameInput.current.value;
                let email = emailInput.current.value;
                let password = passwordInput.current.value;
                let contactNumber = contactInput.current.value;
                let response = await axios.post(Api.doctorSignUp, { doctorName, email, password, contactNumber });
    ...
    const handleSignIn=async ()=>{
        try{     
            if(signInValidation()){
                ...
                let response = await axios.post(Api.doctorSignIn,{email,password});
                sessionStorage.setItem('doctor-details',JSON.stringify(response.data.doctor));
                await axios.post(Api.getDoctorDetails,{doctorId:response.data.doctor._id})
                .then((doctorDetails)=>{
                    sessionStorage.setItem('doctor-extra-details',JSON.stringify(doctorDetails.data.doctorDetail));
                }).catch((err)=>{
                    console.log(err)
                })
